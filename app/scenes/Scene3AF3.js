...
SceneScene3AF3.prototype.initialize = function () {
	...
	$.ajax({
		...
		success: function(data){
				var elementos_columna = 148;	
				for(var i = 254 ;i < data.result[0].xml.resultado.votaciones.votacion.length;i++){
					if (data.result[0].xml.resultado.votaciones.votacion[i].voto=="Sí"){
						elementos_columna++;
						if (elementos_columna > 148 && elementos_columna < 174 ) {
							if (elementos_columna == 149) $("#SceneScene3AF3").append("<div class='col_1'>");
							$(".col_1").append("<h5>"+data.result[0].xml.resultado.votaciones.votacion[i].diputado+"</h5>");
							if (elementos_columna == 173) $("#SceneScene3AF3").append("</div>");
						}else if (elementos_columna > 173) {
							if (elementos_columna == 174) $("#SceneScene3AF3").append("<div class='col_2'>");
